...
export default function ItemBagListPage() {
  ...
  const tableName = 'table_item_bag_list_data';
  const { userProfile } = useStore();
  const selectedTable = userProfile?.data?.id === tableId ? {
    id: tableId,
    name: 'Item Bag List Table',
    type: 'item_bag_list',
  } : undefined;

  const [isFormOpen, setIsFormOpen] = useState(false);
  const [formMode, setFormMode] = useState<FormMode>('add');
  const [selectedRow, setSelectedRow] = useState<NewItemBagListFormData | null>(null);
  ...
  const {
    ...
    handleRowSelection,
    refreshData,
  } = useTableData<ItemBagListFormData>({
    config: {
      tableName,
      columns,
    },
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      <TableHeader
        title={selectedTable?.name || 'Item Bag List Table'}
        description="Manage item bag list data and properties"
        columns={columns}
        filters={filters}
        ...
        >
          <SheetHeader className="px-6 py-4 border-b border-gray-200 dark:border-gray-800">
            <SheetTitle className="bg-gradient-to-r from-purple-600 to-indigo-600 bg-clip-text text-transparent text-2xl font-bold">
              {formMode === 'add' ? 'Add Item Bag List' : 
               formMode === 'edit' ? 'Edit Item Bag List' : 
               'Duplicate Item Bag List'}
            </SheetTitle>
            <SheetDescription className="text-gray-500 dark:text-gray-400">
              {formMode === 'add' ? 'Create a new item bag list with the details below.' :
               formMode === 'edit' ? 'Modify the item bag list values for this entry.' :
               'Create a new entry based on the selected item bag list data.'}
            </SheetDescription>
          </SheetHeader>
