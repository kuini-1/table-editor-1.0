...
export default function MobDataServerTablePage() {
  ...
  const tableName = 'table_mob_data_server';
  const { userProfile } = useStore();
  const selectedTable = userProfile?.data?.id === tableId ? {
    id: tableId,
    name: 'Mob Data Server Table',
    type: 'mob_data_server',
  } : undefined;
  ...
  const {
    ...
    handleRowSelection,
    refreshData,
  } = useTableData<MobDataServerTableRow>({
    config: {
      tableName,
      columns,
    },
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-900">
      <TableHeader
        title={selectedTable?.name || 'Mob Data Server Table'}
        description="Manage mob data server configurations"
        columns={columns}
        filters={filters}
      ...
      <div className="flex-1 overflow-hidden px-4 pb-4">
        <DataTable
          ...
          onDuplicate={(row) => {
            setSelectedRow(row as MobDataServerTableRow);
        ...
        >
          <SheetHeader className="px-6 py-4 border-b border-gray-200 dark:border-gray-800">
            <SheetTitle className="text-indigo-600 dark:text-indigo-400 text-2xl font-bold">
              {formMode === 'add' ? 'Add Mob Data Server Entry' : 
               formMode === 'edit' ? 'Edit Mob Data Server Entry' : 
               'Duplicate Mob Data Server Entry'}
            </SheetTitle>
          </SheetHeader>
