...
import { itemEnchantSchema, type ItemEnchantFormData, columns } from './schema';
import { ItemEnchantForm } from './ItemEnchantForm';

export default function ItemEnchantPage() {
  const searchParams = useSearchParams();
  const tableId = searchParams.get('id') || '';
  const tableName = 'table_item_enchant_data';
  const { userProfile } = useStore();
  const selectedTable = userProfile?.data?.id === tableId ? {
    id: tableId,
    name: 'Item Enchant Table',
    type: 'item_enchant',
  } : undefined;

  const [isFormOpen, setIsFormOpen] = useState(false);
  const [formMode, setFormMode] = useState<FormMode>('add');
  const [selectedRow, setSelectedRow] = useState<ItemEnchantFormData | null>(null);
  ...
  const {
    ...
    handleRowSelection,
    refreshData,
  } = useTableData<ItemEnchantFormData>({
    config: {
      tableName,
      columns,
    },
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      <TableHeader
        title={selectedTable?.name || 'Item Enchant Table'}
        description="Manage item enchant data and properties"
        columns={columns}
        filters={filters}
      ...
      <div className="flex-1 overflow-hidden">
        <DataTable
          ...
          onDuplicate={(row) => {
            const { id, ...rest } = row;
            setSelectedRow({ ...rest, id: '' } as ItemEnchantFormData);
