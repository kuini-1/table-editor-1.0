...
import { columns, type PcTableRow } from './schema';

export default function PcTablePage() {
  const searchParams = useSearchParams();
  const tableId = searchParams.get('id') || '';
  const tableName = 'table_pc_data';
  ...
  const [selectedRow, setSelectedRow] = useState<PcTableRow | null>(null);
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-900">
      <TableHeader
        title={selectedTable?.name || 'PC Table'}
        description="Manage PC data and configurations"
        columns={columns}
        filters={filters}
      ...
      <div className="flex-1 overflow-hidden px-4 pb-4">
        <DataTable
          ...
          onDuplicate={(row) => {
            setSelectedRow(row as PcTableRow);
        ...
        >
          <SheetHeader className="px-6 py-4 border-b border-gray-200 dark:border-gray-800">
            <SheetTitle className="text-indigo-600 dark:text-indigo-400 text-2xl font-bold">
              {formMode === 'add' ? 'Add PC Entry' : 
               formMode === 'edit' ? 'Edit PC Entry' : 
               'Duplicate PC Entry'}
            </SheetTitle>
          </SheetHeader>
