...
import { StatusTransformForm } from './StatusTransformForm';
import { columns, type StatusTransformTableRow } from './schema';
...
export default function StatusTransformTablePage() {
  const searchParams = useSearchParams();
  const tableId = searchParams.get('id') || '';
  const tableName = 'table_status_transform_data';
  const { userProfile } = useStore();
  const selectedTable = userProfile?.data?.id === tableId ? {
    id: tableId,
    name: 'Status Transform Table',
    type: 'status_transform',
  } : undefined;
  ...
  const {
    ...
    handleRowSelection,
    refreshData,
  } = useTableData<StatusTransformTableRow>({
    config: {
      tableName,
      columns,
    },
    ...
    <div className="flex flex-col h-screen bg-gray-900">
      ...
      <div className="flex-1 overflow-hidden px-4 pb-4">
        <DataTable
          ...
          onDuplicate={(row) => {
            setSelectedRow(row as StatusTransformTableRow);
        ...
        >
          <SheetHeader className="px-6 py-4 border-b border-gray-200 dark:border-gray-800">
            <SheetTitle className="text-indigo-600 dark:text-indigo-400 text-2xl font-bold">
              {formMode === 'add' ? 'Add Status Transform Entry' : 
               formMode === 'edit' ? 'Edit Status Transform Entry' : 
               'Duplicate Status Transform Entry'}
            </SheetTitle>
          </SheetHeader>
      ...
      <ImportDialog
        ...
        tableId={tableId}
        tableName="table_status_transform_data"
      />
    </div>
  );
}
