...
export default function WorldmapTablePage() {
  const searchParams = useSearchParams();
  const tableId = searchParams.get('id') || '';
  const tableName = 'table_worldmap_data';
    ...
    <div className="flex flex-col h-screen bg-gray-900">
      ...
      <div className="flex-1 overflow-hidden px-4 pb-4">
        <DataTable
          ...
          onDuplicate={(row) => {
            setSelectedRow(row as WorldmapTableRow);
        ...
        >
          <SheetHeader className="px-6 py-4 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-800">
            <SheetTitle className="text-indigo-600 dark:text-indigo-400 text-2xl font-bold">
              {formMode === 'add' ? 'Add Worldmap Entry' : 
               formMode === 'edit' ? 'Edit Worldmap Entry' : 
               'Duplicate Worldmap Entry'}
            </SheetTitle>
          </SheetHeader>
