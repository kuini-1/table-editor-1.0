...
import { columns, type ExpTableRow } from './schema';

export default function ExpTablePage() {
  const searchParams = useSearchParams();
  const tableId = searchParams.get('id') || '';
  const tableName = 'table_exp_data';
    ...
    <div className="min-h-screen bg-gray-900">
      ...
      <div className="flex-1 overflow-hidden">
        <DataTable
          ...
          onDuplicate={(row) => {
            setSelectedRow(row as ExpTableRow);
        ...
        >
          <SheetHeader className="px-6 py-4 border-b border-gray-200 dark:border-gray-800">
            <SheetTitle className="bg-gradient-to-r from-purple-600 to-indigo-600 bg-clip-text text-transparent text-2xl font-bold">
              {formMode === 'add' ? 'Add Experience Entry' : 
               formMode === 'edit' ? 'Edit Experience Entry' : 
               'Duplicate Experience Entry'}
            </SheetTitle>
            <SheetDescription className="text-gray-500 dark:text-gray-400">
              {formMode === 'add' ? 'Create a new experience entry with the details below.' :
               formMode === 'edit' ? 'Modify the experience values for this entry.' :
               'Create a new entry based on the selected experience data.'}
            </SheetDescription>
          </SheetHeader>
